...
nx g @nrwl/node:application --frontendProject=client --dry-run

# npm module 'apollo' have to be installed 'yarn add apollo --dev'
# Command to push schema - Apollo Manager
yarn apollo service:push --endpoint=http://localhost:4000/ --config=./apps/server/apollo.config.js

# Command to check schema - Apollo Manager
yarn apollo service:check --endpoint=http://localhost:4000/ --config=./apps/server/apollo.config.js
